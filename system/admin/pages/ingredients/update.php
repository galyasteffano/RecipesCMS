<?php
    ...
    $id=$_GET['id'];
    $result=getSingleItem('ingredient_name','tbl_ingredient','ingredient_id',$id);
    ...
    if ($_SERVER['REQUEST_METHOD']=='POST'){
        if ($_POST['update_btn']==='cancel') 
            ...
            $updated=updateValue('ingredient_name',$new_value,'tbl_ingredient','ingredient_id',$id);
            if(!$updated)
                redirectTo('0');
            redirectTo('1');
         }
...
<?php $data=mysqli_fetch_assoc($result); ?>
    ...
    <form action="<?php echo 'update.php?id='.$id; ?>" method="post"> 
       <input type="text" name="new-value" value="<?php echo $data['ingredient_name']; ?>"> 
